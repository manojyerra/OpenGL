...
void GameLoop::CheckBallCollision(Ball* ball1, Ball* ball2)
{
	...
	if(dist <= r1+r2)
	{
		...
		if(newDist < dist)
		{
			...
			float prevBall1Speed = b1->GetSpeed();
			float prevBall2Speed = b2->GetSpeed();

			b1->SetVelocity(nb1vx, nb1vy);
			b2->SetVelocity(nb2vx, nb2vy);

			float ball1SpeedPerCent = (b1->GetSpeed() * 100) / prevBall1Speed;
			float ball2SpeedPerCent = (b2->GetSpeed() * 100) / prevBall2Speed;
			...
			b1->SetRotSpeedPercent(ball1SpeedPerCent);
			b2->SetRotSpeedPercent(ball2SpeedPerCent);
		}
	}
}
