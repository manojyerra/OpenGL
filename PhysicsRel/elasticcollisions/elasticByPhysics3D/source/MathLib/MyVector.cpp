...
float MyVector::GetProjValue(MyVector* myVec)
{
	if(myVec->Length == 1)
	{
		return(p.x*myVec->p.x + p.y*myVec->p.y +  p.z*myVec->p.z);
	}
	else
	{
		float saveX = myVec->p.x;
		float saveY = myVec->p.y;
		float saveZ = myVec->p.z;

		myVec->Normalize();
		float returnValue = p.x*myVec->p.x + p.y*myVec->p.y +  p.z*myVec->p.z;
...
MyVector MyVector::Cross(MyVector* myVec)
{
	float x =  ( p.y*myVec->p.z - myVec->p.y*p.z );
	float y = -( p.x*myVec->p.z - myVec->p.x*p.z );
	float z =  ( p.x*myVec->p.y - myVec->p.x*p.y );
