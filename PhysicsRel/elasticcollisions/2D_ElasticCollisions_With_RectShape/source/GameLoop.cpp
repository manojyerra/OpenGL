...
void GameLoop::Init()
{
	...
	RectShape* rectShape = new RectShape(200,400);
	_rectShapeVec.push_back(rectShape);

	rectShape = new RectShape(400,400);
	_rectShapeVec.push_back(rectShape);
}
...
void GameLoop::Update()
{
	...
	{
		Ball* ball = _rectShapeVec[0]->ball1;

		float dx = (Input::MouseX - ball->GetCX()) / 4.0f;
		float dy = (Input::MouseY - ball->GetCY()) / 4.0f;
		...
		MyVector* velVec = ball->GetVelocityVec();
		velVec->p.Add( &newVel.p );
	}

	for(int i=0; i<_rectShapeVec.size();i++)
	{
		CollisionWithBorder(_rectShapeVec[i]->ball1);
		CollisionWithBorder(_rectShapeVec[i]->ball2);
	}

	for(int i=0; i<_rectShapeVec.size();i++)
	{
		CheckBallCollision(_rectShapeVec[i]->ball1, _rectShapeVec[i]->ball2);

		for(int j=i+1; j<_rectShapeVec.size();j++)
		{
			CheckBallCollision(_rectShapeVec[i]->ball1, _rectShapeVec[j]->ball1);
			CheckBallCollision(_rectShapeVec[i]->ball1, _rectShapeVec[j]->ball2);
			CheckBallCollision(_rectShapeVec[i]->ball2, _rectShapeVec[j]->ball1);
			CheckBallCollision(_rectShapeVec[i]->ball2, _rectShapeVec[j]->ball2);
	...
	for(int i=0; i<_rectShapeVec.size();i++)
	{
		_rectShapeVec[i]->Update();
	}

	for(int i=0; i<_rectShapeVec.size();i++)
	{
		_rectShapeVec[i]->ResetDist();
	}
}
