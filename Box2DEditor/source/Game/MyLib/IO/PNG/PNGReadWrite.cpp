...
unsigned char* PNGReadWrite::Read(char* buf, int bufSize, int* imgWidth, int* imgHeight, int* bytesPerPixel)
{
	if(buf == NULL)
		return NULL;
    
	myFileOpen((unsigned char*)buf, bufSize);
    png_byte header[8]; //8 is the maximum header size that can be checked

	myFileRead(header, 1, 8);
	...
	png_init_io(png_ptr);		//init png reading
