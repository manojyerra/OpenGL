	...
	if(_pos < (int)_fileData.length())
	{
		...
		for(; i<(int)_fileData.length(); i++)
		{
			if(_fileData[i] == '\n')
			{
				...
				if(i-1 >= 0 && (int)_fileData[i-1] == 13)
					return _fileData.substr(temp, i-temp-1);
				else
...
string FileReader::GetTagStringData(string tag, bool trim)
{
	vector<string> strVec = GetTagData(tag, trim);
	if(strVec.size() != 1)
	{
		int a=1,b=0,c;
		c = a/b;
